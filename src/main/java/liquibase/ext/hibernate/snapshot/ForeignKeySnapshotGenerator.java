...
public class ForeignKeySnapshotGenerator extends HibernateSnapshotGenerator {
	...
	protected void addTo(DatabaseObject foundObject, DatabaseSnapshot snapshot)
		...
		if (foundObject instanceof Table) {
			...
			HibernateDatabase database = (HibernateDatabase) snapshot
					.getDatabase();
			Configuration cfg = database.getConfiguration();
			Iterator<org.hibernate.mapping.Table> tableMappings = cfg
					.getTableMappings();
			while (tableMappings.hasNext()) {
				org.hibernate.mapping.Table hibernateTable = (org.hibernate.mapping.Table) tableMappings
						.next();
				Iterator fkIterator = hibernateTable.getForeignKeyIterator();
				while (fkIterator.hasNext()) {
					org.hibernate.mapping.ForeignKey hibernateForeignKey = (org.hibernate.mapping.ForeignKey) fkIterator
							.next();
					Table currentTable = new Table().setName(hibernateTable
							.getName());
					currentTable.setSchema(hibernateTable.getCatalog(),
							hibernateTable.getSchema());
					org.hibernate.mapping.Table hibernateReferencedTable = hibernateForeignKey
							.getReferencedTable();
					Table referencedTable = new Table()
							.setName(hibernateReferencedTable.getName());
					...
					if (hibernateForeignKey.isPhysicalConstraint()) {
						...
						for (Object column : hibernateForeignKey.getColumns()) {
							fk.addForeignKeyColumn(((org.hibernate.mapping.Column) column)
									.getName());
						}
						fk.setForeignKeyTable(currentTable);
						for (Object column : hibernateForeignKey
								.getReferencedColumns()) {
							fk.addPrimaryKeyColumn(((org.hibernate.mapping.Column) column)
									.getName());
						}
						if (fk.getPrimaryKeyColumns() == null
								|| fk.getPrimaryKeyColumns().isEmpty()) {
							for (Object column : hibernateReferencedTable
