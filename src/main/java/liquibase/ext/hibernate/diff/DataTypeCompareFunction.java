...
class DataTypeCompareFunction extends ToStringCompareFunction {
	...
	public boolean areEqual(Object referenceValue, Object compareToValue) {
		...
		String referenceTypeString = referenceValue.toString()
				.toUpperCase();
		String compareToTypeString = compareToValue.toString()
				.toUpperCase();

		if ((referenceTypeString.contains("(") || compareToTypeString
				.contains("("))
				&& (!referenceTypeString.contains("(") || !compareToTypeString
						.contains("("))) {
			if (referenceTypeString.contains("("))
				referenceTypeString = referenceTypeString.substring(0,
						referenceTypeString.indexOf('('));
			if (compareToTypeString.contains("("))
				compareToTypeString = compareToTypeString.substring(0,
						compareToTypeString.indexOf('('));
		}

		// CHAR = CHARACTER
		if (referenceTypeString.startsWith("CHARACTER"))
			referenceTypeString = referenceTypeString.replaceFirst(
					"CHARACTER", "CHAR");
		if (compareToTypeString.startsWith("CHARACTER"))
			compareToTypeString = compareToTypeString.replaceFirst(
					"CHARACTER", "CHAR");
